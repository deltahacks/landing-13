...
function HeroCopy({ applicationsOpen }: { applicationsOpen: boolean }) {
  return (
    <>
      <h1 className="font-display absolute top-[37.81vw] left-[calc(50%+5px)] z-30 m-0 -translate-x-1/2 text-[55px] leading-none font-normal tracking-[-0.4px] whitespace-nowrap text-[#32464c] md:top-[10.625vw] md:left-1/2 md:text-[clamp(40px,4.861vw,70px)] md:tracking-[-0.0486vw]">
        DeltaHacks 13
      </h1>
      <p className="absolute top-[50.25vw] left-[calc(50%+5px)] z-30 m-0 -translate-x-1/2 text-[18px] leading-normal font-bold whitespace-nowrap text-[#303a3d] md:top-[16.667vw] md:left-1/2 md:text-[clamp(12px,1.25vw,18px)]">
        McMaster&apos;s Annual Hackathon for Change
      </p>
      <p className="absolute top-[54.98vw] left-[calc(50%+5px)] z-30 m-0 -translate-x-1/2 text-[18px] leading-normal font-bold whitespace-nowrap text-[#303a3d] md:top-[18.542vw] md:left-1/2 md:text-[clamp(12px,1.25vw,18px)]">
        Jan 10-11, 2027 • In Person
      </p>
      {applicationsOpen && <ApplyButton />}
    </>
  );
}

async function areApplicationsOpen(): Promise<boolean> {
  try {
